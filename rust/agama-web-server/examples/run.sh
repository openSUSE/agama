...
curl -X PUT -H "Content-Type: application/json" http://localhost:3000/dbus/call \
...
curl -X GET -H "Content-Type: application/json" http://localhost:3000/dbus/properties \
	-d @examples/get_property.json
echo

echo "Set the locale (using properties Set)"
curl -X PUT -H "Content-Type: application/json" http://localhost:3000/dbus/properties \
